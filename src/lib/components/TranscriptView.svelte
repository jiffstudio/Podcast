<script lang="ts">
  ...
  import { transcript, currentTime, isThinking, setTime } from '$lib/stores/player';
  ...
  // Auto-scroll
  $: if (activeLineIndex !== -1 && container) {
      document.getElementById(`line-${activeLineIndex}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' });
  }
</script>

<div bind:this={container} class="flex-1 overflow-y-auto p-8 md:p-16 scroll-smooth pb-40">
  <div class="max-w-3xl mx-auto space-y-8">
    {#each $transcript as line, i}
      <div id="line-{i}" 
           ...
           on:click={() => setTime(line.seconds)}
           on:keydown={e => e.key === 'Enter' && setTime(line.seconds)}
           role="button"
           tabindex="0">
